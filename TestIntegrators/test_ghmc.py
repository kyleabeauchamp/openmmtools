...
def test_hmc(timestep, steps_per_hmc, alpha):
    ...
    integrator = integrators.GHMCIntegrator2(temperature=temperature, nsteps=steps_per_hmc, timestep=timestep)
    context = mm.Context(testsystem.system, integrator)
    context.setPositions(testsystem.positions)
    integrator.step(n_steps)
    state = context.getState(getPositions=True)
    xyz = state.getPositions(asNumpy=True)
    return integrator.acceptance_rate, xyz

timestep_list = np.array([2.0, 2.1])
...
for i, timestep in enumerate(timestep_list):
    for j, alpha in enumerate(alpha_list):
        ...
        acceptance, xyz = test_hmc(timestep, steps_per_hmc, alpha)
